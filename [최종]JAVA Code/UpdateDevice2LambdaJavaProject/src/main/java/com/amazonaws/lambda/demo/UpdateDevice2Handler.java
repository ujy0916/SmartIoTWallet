...
/*
 * UpdateDevice2Function
 * 어플에서 비활성화 할 수 있도록 디바이스 쉐도우의 desired 부분을 변경해주는 람다함수.
 * /devices/{device}/value - PUT - 메서드 실행
 */

public class UpdateDevice2Handler implements RequestHandler<Event, String> {
    ...
    private String getPayload(ArrayList<Tag> tags) {
        ...
        for (int i=0; i < tags.size(); i++) {
          ...
          //어플에서 PUT을 한 값을 </devices/{device}/value-PUT> 의 매핑 탬플릿을 이용하여 tagName, tagValue를 가져와
        }
        return String.format("{ \"state\": { \"desired\": { %s } } }", tagstr);
      //값을 섀도우에 desired 형태로 반환해줌.
    }
