...
public class UploadRFIDValueHandler implements RequestHandler<Document, String> {
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        String state = "";
        if(document.current.state.reported.State.equals("1")) {state = "OPEN";}  //아두이노에서 보낸 State의 값이 1이면 지갑은 열림상태
        else if(document.current.state.reported.State.equals("0")) {state = "CLOSE";} //아두이노에서 보낸 State의 값이 0이면 지갑은 닫힘상태

        //RFID_value DB에 RFID, time, 지갑의 여닫이 상태, timestamp 항목을 저장.
...
/**
 * AWS IoT은(는) 섀도우 업데이트가 성공적으로 완료될 때마다 /update/documents 주제에 다음 상태문서를 게시함.
 * JSON 형식의 상태문서는 2개의 기본 노드를 포함함->previous 및 current. 
 * previous 노드에는 업데이트가 수행되기 전의 전체 섀도우 문서의 내용이 포함되고, 
 * current에는 업데이트가 성공적으로 적용된 후의 전체 섀도우 문서가 포함됨.
 * 섀도우가 처음 업데이트(생성)되면 previous 노드에는 null이 포함됨.
 * 
 * timestamp는 상태문서가 생성된 시간 정보이고, device는 상태문서에 포함된 값이 아닌 Iot규칙을 통해서 Lambda함수로 전달된 값임. 
 * 이 값을 해당 규칙과 관련된 사물이름을 나타냄. 
 */
class Document {
    public Thing previous;       
    public Thing current;
    public long timestamp;
    public String device;       // AWS IoT�� �깅��� �щЪ �대� 
    //public String RFID;
}
